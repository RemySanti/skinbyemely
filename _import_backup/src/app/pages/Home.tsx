...
import { Award, Shield, Star, Clock, ShoppingBag, ExternalLink, Loader2, Lock } from 'lucide-react';
import { ImageWithFallback } from '../components/figma/ImageWithFallback';
import { useState, useEffect } from 'react';
import { catalogService, SquareProduct } from '../services/squareService';
import { openExternalLink } from '../utils/navigation';
import { stripHtml } from '../components/ui/utils';
import { FEATURED_PRODUCT_IDS } from '../config/featured-products';
...
import facialImage from 'figma:asset/52e70ed822f015234a068fbbac1d71aa8bb344e3.png';
import facial60MinImage from 'figma:asset/14693d6b6a79abf064017892579c45e84ce5b4e4.png';
...
import backMicrodermabrasionImage from 'figma:asset/90bc33beceff281af925ef00b069989d0783ff3b.png';
...
import { fetchServicePrices, getDefaultServicePrices, ServicePrice } from '../config/service-prices';

export default function Home() {
  const [featuredProducts, setFeaturedProducts] = useState<StripeProduct[]>([]);
  const [productsLoading, setProductsLoading] = useState(true);
  const [servicePrices, setServicePrices] = useState<ServicePrice[]>(getDefaultServicePrices());
  ...
  useEffect(() => {
    ...
    fetchServicePrices().then(setServicePrices).catch(() => {});
  }, []);
      ...
      <section className="py-24 px-4 bg-white">
        <div className="max-w-7xl mx-auto">
          <FadeInSection className="text-center mb-12">
            ...
            <p className="text-lg text-[#6b6b6b] max-w-2xl mx-auto">
              Custom treatments designed for visible, lasting results
            </p>
          </FadeInSection>

          <FadeInSection delay={0.2} className="px-4 md:px-12">
            <Carousel
              opts={{
                align: "start",
                loop: true,
            ...
            >
              <CarouselContent className="-ml-4">
                {(() => {
                  // Only show the 5 main signature services
                  ...
                  // Filter to only include main services
                  const mainServices = servicePrices.filter(service => 
                    mainServiceIds.includes(service.id)
                  );

                  // Map service IDs to their images
                  const serviceImages: Record<string, string> = {
                    'new-client-facial': newClientFacialImage,
                    'microdermabrasion': microdermabrasionImage,
                    'dermaplane-deluxe': 'https://images.unsplash.com/photo-1570172619644-dfd03ed5d881?q=80&w=1200&auto=format&fit=crop',
                    'cranberry-peel': cranberryPeelImage,
                    'nanoneedling': nanoneedlingImage,
                  };

                  return mainServices.map((service, index) => {
                    const isBack = (service.title || '').toLowerCase().includes('back') && service.id === 'microdermabrasion';
                    const imageSrc = isBack ? backMicrodermabrasionImage : (serviceImages[service.id] || '');

                    return (
                    <CarouselItem key={`${service.id}-${index}`} className="md:basis-1/2 lg:basis-1/3 pl-4">
                      <Link to={service.link} className="block h-full">
                        <div className="group relative overflow-hidden border border-[#b8956a]/15 h-full bg-white flex flex-col hover:shadow-lg transition-shadow duration-300">
                          <div className="aspect-[4/3] relative overflow-hidden">
                            ...
                            <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end text-white">
                                <span className="font-medium bg-black/30 backdrop-blur-sm px-3 py-1 rounded text-sm">{service.duration}</span>
                                <span className="font-medium bg-[#b8956a] px-3 py-1 rounded text-sm">{service.price}</span>
                            </div>
                          </div>
                          <div className="p-6 flex flex-col flex-grow">
                            <h3 className="text-xl font-serif text-[#2c2c2c] mb-3 group-hover:text-[#b8956a] transition-colors">{service.title}</h3>
                            <p className="text-[#6b6b6b] mb-6 leading-relaxed text-sm flex-grow line-clamp-3">{service.description}</p>
                            <Button className="w-full btn-outline-bronze rounded py-2 group-hover:bg-[#b8956a] group-hover:text-white group-hover:border-[#b8956a] transition-all text-sm">
                              Learn More
                            </Button>
                          </div>
                        </div>
              ...
              <CarouselPrevious className="border-[#b8956a] text-[#b8956a] hover:bg-[#b8956a] hover:text-white" />
              <CarouselNext className="border-[#b8956a] text-[#b8956a] hover:bg-[#b8956a] hover:text-white" />
            </Carousel>
          </FadeInSection>
          ...
          <FadeInSection delay={0.4} className="text-center mt-12">
            <a
              href="https://book.squareup.com/appointments/f7dcst2ljp85dq/location/LMVSQK9C6PR4T/services"
              target="_blank"
              rel="noopener noreferrer"
            >
              <Button className="btn-outline-bronze px-10 py-4 rounded hover:shadow-md transition-all">
                View All Services
              </Button>
            </a>
          </FadeInSection>
